...
class GoogleSyncHub:
    ...
    async def synchronize_site_telemetry(
        ...
        is_token_invalid = not access_token or access_token in ("", "none", "invalid_token", "expired_token")

        if is_token_invalid:
            gsc_connected = False
            gsc_error = "OAuth erişim anahtarı bulunamadı veya süresi dolmuş."
            ga4_connected = False
            ga4_error = "OAuth erişim anahtarı bulunamadı veya süresi dolmuş."
        else:
        ...
        total_clicks = sum(r.clicks for r in gsc_rows) if gsc_connected else 0
        total_impressions = sum(r.impressions for r in gsc_rows) if gsc_connected else 0
        avg_ctr = (total_clicks / total_impressions) if total_impressions > 0 else 0.0
        avg_position = (sum(r.position for r in gsc_rows) / len(gsc_rows)) if gsc_rows and gsc_connected else 0.0

        total_users = sum(r.active_users for r in ga4_rows) if ga4_connected else 0
        ...
        if ga4_connected and weighted_bounce > 0.40:
            insights.append({
        ...
        if ga4_connected and organic_cvr > 3.0:
            insights.append({
